---
title: 构建私有kaniko-executor
date: 2021-10-12 09:47
categories:
- docker
tags:
- kaniko
...
摘要: 构建私有kaniko-executor
<!-- more -->

kaniko是在容器或Kubernetes集群内部通过Dockerfile构建容器镜像的工具。<br/>
kaniko不依赖Docker守护程序，而是完全在用户空间中执行Dockerfile中的每个命令。<br/>
性能要比docker in docker高<br/>
官方镜像`gcr.io/kaniko-project/executor`在国内无法下载，这里依赖`aiotceo/kaniko-executor:v1.3.0`制作一个自用debug版本kaniko-executor。用于gitlab CI构建镜像
