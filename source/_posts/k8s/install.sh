...
network.k8s() {
  ...
  kubectl apply -f https://projectcalico.docs.tigera.io/v3.20/manifests/calico.yaml
...
main() {
  ...
  # version="1.24.1"
  if ! version=$(whiptail --title "创建k8s" --menu "请您选择k8s版本" 10 60 4 \
    "1.24.1" "1.24.1" \
    "-" "..." 3>&1 1>&2 2>&3); then
    whiptail --title "创建k8s" --msgbox "创建k8s终止..." 10 60
    exit 1
  fi

  if ! role=$(whiptail --title "创建k8s" --menu "请您选择node的角色" 10 60 4 \
    "Master" "Master" \
    "Node" "Node" 3>&1 1>&2 2>&3); then
    whiptail --title "创建k8s" --msgbox "创建k8s终止..." 10 60
    exit 1
  fi

  if ! reposType=$(whiptail --title "创建k8s" --menu "请您选择node的角色" 10 60 4 \
    "ali" "ali" \
    "local" "local" 3>&1 1>&2 2>&3); then
    whiptail --title "创建k8s" --msgbox "创建k8s终止..." 10 60
    exit 1
  fi

  change.repos.ali "$reposType"
  selinux.config
  swap.config
  sysctl.k8s
  sysctl.ipvs
  ...
  if [ "$role" = "Master" ]; then
    ...
    if ! code=$(whiptail --title "初始化K8S" --inputbox "请将kubeadm join 填在输入框中" 10 60 3>&1 1>&2 2>&3); then
      whiptail --title "创建k8s" --msgbox "创建k8s终止..." 10 60
      exit 1
    else
      initial.k8s.node "${code}"
